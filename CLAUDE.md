...
- `.claude/settings.json` must contain only safe, shared, non-secret project settings.
- Never copy `.claude/settings.local.json` into committed project config.
- Put reusable repo workflow guidance in `.claude/skills/` instead of burying it in ad hoc prompts.
